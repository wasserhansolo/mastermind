...
int main(int argc, char *argv[])
{
    ...
    /* MEINE IMPLEMENTATION ANFANG */
    
    
	int sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	if (sockfd < 0) {
		bail_out(EXIT_FAILURE, "socket creation failure");
	}
    
    int onebyte = 0;
    if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR,&onebyte, sizeof(onebyte)) < 0) {
		bail_out(EXIT_FAILURE, "setsockopt_SO_REUSEADDR failure");
	}
    ...
    if (bind(sockfd, (struct sockaddr *) &binding_address, sizeof(binding_address)) < 0) {
		bail_out(EXIT_FAILURE, "bind failure");
	}
	
	if (listen(sockfd, BACKLOG) < 0) {
		bail_out(EXIT_FAILURE, "listen failure");
	}
